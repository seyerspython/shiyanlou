...
def read_udata(userdatafile):
        u_data=UserDate(userdatafile)
        for us in u_data.userdata
            data=[us]
            data.append[u_data.userdata[us]]
            listqueue1.put(data)
def cal_tax_sp(config):
        data=queue1.get()
        queue2.put(newdata)
...
if __name__=='__main__':
    if len(sys.argv)>0 :
            ...
            p1=Process(target=read_udata,args(userdaefile,))
            p2=Process(target=cal_tax_sp,args(config,))
            p3=Process(target=writefile,args(outputfile,))
            p1.start()
            p2.start()
            p3.start()
